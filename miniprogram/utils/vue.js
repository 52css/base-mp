...
function deepClone(obj) {
  // 如果是原始类型或者 null，则直接返回
  if (obj === null || typeof obj !== "object") {
    return obj;
  }

  // 根据 obj 的类型创建一个新的空对象或数组
  const clone = Array.isArray(obj) ? [] : {};

  // 遍历 obj 的属性，并递归调用 deepClone 复制每个属性
  for (let key in obj) {
...
function watch(source, cb, options = {}) {
  ...
  if (options.immediate) {
    ...
    // 手动调用副作用函数，拿到的值就是旧值
    oldValue = deepClone(unRef(effectFn()));
  }
}
  ...
  const splitFieldsAndMethods = (obj) => {
    ...
    for (const k in obj) {
      if (typeof obj[k] === "function") {
        ...
        // 如果是对象是引用类型，context.data[k] 永远相等，先不判断，直接赋值，性能优化交给微信小程序
        // if (!equals(obj[k], context.data[k])) {
        fields[k] = obj[k];
        // }
      }
    }
...
function createProps(context, props) {
  return new Proxy(props, {
    ...
    set: function (target, key, value, receiver) {
      ...
      // 发送自定义事件，传递数据
      context.triggerEvent("update" + key, { value });
      return Reflect.set(target, key, value, receiver);
    },
  });
}
  ...
  Component({
    ...
    lifetimes: {
      ...
      attached() {
        ...
        this.emit = emit;
        useHook(this, hook.bind(this, setupProps, this));
      },
      ready(...args) {
...
[...pageLifetimes, ...lifetimes].forEach((lifetimesKey) => {
  rtv[lifetimesKey] = (hook) => {
